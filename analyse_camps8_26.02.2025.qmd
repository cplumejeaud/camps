...
row.names(tododf) <- (camps |> filter(is.na(str_match(camps$nom, pattern='CADA'))))$unique_id 
...
```{r}
campsSansCada <- camps |> filter(is.na(str_match(camps$nom, pattern='CADA')))
...
##### Créer un design avec survey (sans strates ni grappes ici 

Les jumeaux valent 1/n de leur nombre s'ils sont n jumeaux pour un même camps.

Les vrais comptent beaucoup plus que les simulés (qui sont très nombreux, presque 8 fois plus)

```{r}
# |> filter (statut_camp=='sim')
# simulation <- simulation |> 
#   add_count(parent_id, name = "poids") |>
#   mutate(poids = as.numeric(poids) )
# 
# simulation <- simulation |>
#   mutate(poids = ifelse(statut_camp =='sim', 1, poids-1)) 

simulation <- simulation |>
  add_count(statut_camp, name = "poids") |>
  mutate(poids = as.numeric(poids) ) |>
  mutate(poids =  1/(2*poids))
  

simulation |> arrange(parent_id) |>  filter (parent_id == 1088) |> head(10)
 
library(survey)
simulationw <- svydesign(ids = ~1, data = simulation, weights = ~poids)
```

##### Version stratifiée sur les villes (ville_proche_nom) 
...
sim <- simulation |>
  add_count(ville_proche_nom, name = "poids") |>
  mutate(poids = as.numeric(poids) ) |>
  filter(poids > 1) ## Eviter le cas ou pas de jumeau dans la même ville_proche_nom

sim <- sim |>
  mutate(poids = ifelse(statut_camp =='sim', 1, poids-1)) 


sim |> arrange(ville_proche_nom) |>  filter (ville_proche_nom == 'FUA of Lyon') |> head(10)
 
library(survey)
simulationw <- svydesign(ids = ~1, strata=~ville_proche_nom, data = sim, weights = ~poids)
...
#### Analyse de variance pondérée à 1 facteur
...
p-value = 0.0005041 (presque 0) donc H0 (hypothèse d'indépendance est rejetée sous un risque faible ( 0.0005041 ) donc H1 est acceptée : il y a une liaison entre les variables distance aux mines et vrai camps ou camps simulé.

Les camps simulés ont tendance à être plus loin des mines et des décharges.

#### On rejoue la même vérif pour distance_121_zi_zac
...
1.  sélectionner les vrai avec degurba = 1,

2.  puis récupérer les simulations correspondantes qui tombent dans le même degré d'urbanisation

3.  Echantillon stratifié par parent_id ? (la somme des poids ne fait pas 1). Ou bien par ville_proche_nom ?

##### Design à faire vérifier aussi

```{r}
#fua=='F'
#degurba==1
urban <- simulation |> filter(degurba==1 & statut_camp=='vrai') |> mutate(unique_id = as.integer(unique_id)) |> select (unique_id)

urbanSim <- simulation |> filter(degurba==1 & parent_id %in% urban$unique_id)
...
testw <- svydesign(ids = ~1, strata=~ville_proche_nom, data = test, weights = ~poids)
...
# sim   20.82  14.69   3.64   5.22  -7.90 -14.46 -12.75   1.20  -3.57  -7.22   0.47   1.50   0.47   1.34  -1.37
# vrai -20.82 -14.69  -3.64  -5.22   7.90  14.46  12.75  -1.20   3.57   7.22  -0.47  -1.50  -0.47  -1.34   1.37

## Resultats pondérés et filtrés par degurba : 2, 3, 111
#            clc_majoritaire_13_mixte
# statut_camp     2     3     4     5   111   112   121   122   123   124   131   132   133   141   142
#        sim  16.48 13.68  2.36  2.71 -5.11 -2.66 -2.74  0.04 -2.36 -3.76  2.18  2.36  0.34  3.05  0.78
#        vrai -7.44 -6.17 -1.07 -1.22  2.31  1.20  1.24 -0.02  1.06  1.70 -0.99 -1.07 -0.15 -1.38 -0.35
...
Compil2 <- as.data.frame(croisement)
Compil2 <- Compil2 |> 
  left_join(as.data.frame(round(res$residuals, 2))|> rename(residus = Freq), by = c("statut_camp" = "statut_camp", "clc_majoritaire_13_mixte" = "clc_majoritaire_13_mixte"))  
Compil2$filtre<- 'degurba2'
Compil2


Compil <- rbind(Compil, Compil2)

Compil |> arrange(clc_majoritaire_13_mixte, filtre, statut_camp)
```

Si degurba = 3

```{r}
urban <- simulation |> filter(degurba==3 & statut_camp=='vrai') |> mutate(unique_id = as.integer(unique_id)) |> select (unique_id)

urbanSim <- simulation |> filter(degurba==3 & parent_id %in% urban$unique_id)
...
```{r}
write.table(Compil, "./croisements_clc_degurba_identiques.csv", row.names = F)

```

```{r}
types_de_fua = c('C', 'K', 'F', 'H');

Compil <- data.frame()

for (type in types_de_fua) {
  urban <- simulation |> filter(fua==type & statut_camp=='vrai') |> mutate(unique_id = as.integer(unique_id)) |> select (unique_id)
  
  urbanSim <- simulation |> filter(fua==type & parent_id %in% urban$unique_id)
  ...
  Compil2 <- as.data.frame(croisement)
  Compil2 <- Compil2 |> 
    left_join(as.data.frame(round(res$residuals, 2))|> rename(residus = Freq), 
              by = c("statut_camp" = "statut_camp", 
                     "clc_majoritaire_13_mixte" = "clc_majoritaire_13_mixte"))  
  Compil2$filtre<- type
  #Compil2
  Compil <- rbind(Compil, Compil2)
}
...
Compil |> arrange(clc_majoritaire_13_mixte, filtre, statut_camp)
write.table(Compil|> arrange(clc_majoritaire_13_mixte, filtre, statut_camp), 
            "./croisements_clc_fua_identiques.csv", row.names = F)
```

```{r}
types_de_degurba = c(1,2,3);

Compil <- data.frame()

for (type in types_de_degurba) {
  urban <- simulation |> filter(degurba==type & statut_camp=='vrai') |> mutate(unique_id = as.integer(unique_id)) |> select (unique_id)
  
  urbanSim <- simulation |> filter(degurba==type & parent_id %in% urban$unique_id)
...
Compil |> arrange(clc_majoritaire_13_mixte, filtre, statut_camp)
write.table(Compil|> arrange(clc_majoritaire_13_mixte, filtre, statut_camp), 
            "./croisements_clc_degurba_identiques.csv", row.names = F)
